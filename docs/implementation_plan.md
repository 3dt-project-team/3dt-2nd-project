...
데이터 저장소: ADLS Gen2 (레이크) + Azure SQL Database (서빙).

## 마일스톤

### M1: 인프라 설정
- [ ] Azure Key Vault 생성 및 시크릿 등록 (`adls-account-name`, `sql-connection-string` 등)
- [ ] ADLS Gen2 컨테이너 구성 (`raw` / `curated` / `feature`)
- [ ] Azure SQL Database 스키마 정의
- [ ] Managed Identity 권한 설정 (ADLS, SQL, Key Vault)

### M2: 데이터 수집 (ADF)
- [ ] Linked Service 연결 구성 (ADLS, SQL, Key Vault)
