...
# [셀 3] 피처 엔지니어링
# TODO: 실제 피처 추출 로직 구현
#   - 제목 + 본문 합산 텍스트 컬럼 생성 (text_for_model)
#   - 감성 라벨 컬럼 (positive/negative/neutral) — 초기엔 규칙 기반
#   - 반도체 키워드 포함 여부 플래그
#   - 발행 날짜 → 연/월/주 파티션 컬럼
# ==============================================================================
...
# df_feature.write.mode("overwrite").partitionBy("pub_year", "pub_month").parquet(feature_path)
# print(f"[OK] {df_feature.count()}건 저장 완료: {feature_path}")

# ==============================================================================
# [셀 5] (선택) PostgreSQL에 요약 통계 적재
# ==============================================================================
# import pandas as pd
# engine = vault.get_pg_connection(engine="sqlalchemy")
# summary = df_feature.groupBy("pub_year", "pub_month").count().toPandas()
