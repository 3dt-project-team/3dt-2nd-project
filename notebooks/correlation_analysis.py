...
# MAGIC %md
# MAGIC # SENSE 프로젝트 — 전통적 통계 분석 노트북 (Baseline Comparison)
# MAGIC
# MAGIC > TimesFM과 동일 데이터·피처·백테스트 프레임워크를 사용하되,
...
# MAGIC ---
# MAGIC **변경 이력**
# MAGIC - v2 (2025-04-11): `timesfm_inference_0411.py`와 1:1 대응하도록 전면 재작성
# MAGIC   - 동일 데이터 로드 / 피처 마트 / 파생 변수 / 시나리오 / 백테스트 프레임워크
# MAGIC   - Granger Causality, Permutation Importance, Conformal PI 추가
# MAGIC   - PostgreSQL 출력 포맷 통일 (`fact_stat_forecast`)
...
macro_cols = [c for c in macro.columns if c not in ("date", "Unnamed: 0")]

raw_semi["date"] = pd.to_datetime(raw_semi["date"])
semi = raw_semi.sort_values("date").drop_duplicates("date")
semi_cols = [c for c in semi.columns if c not in ("date", "Unnamed: 0")]
...
for ticker in TICKERS:
    kfin_ticker = raw_kfin[raw_kfin["ticker"] == ticker].copy()
    daily = kfin_ticker.sort_values("date").drop_duplicates("date")
    daily = daily.merge(macro, on="date", how="left")
    daily = daily.merge(semi, on="date", how="left")
    daily = daily.sort_values("date").reset_index(drop=True)
    for col in macro_cols + semi_cols:
        if col in daily.columns:
            daily[col] = pd.to_numeric(daily[col], errors="coerce")
            daily[col] = daily[col].ffill()
    daily.dropna(subset=["close"], inplace=True)
    feature_marts[ticker] = daily
    print(f"[{TICKER_NAMES[ticker]}] 피처 마트: {daily.shape}")
...
# MAGIC # 3. 파생 변수 생성
# MAGIC
# MAGIC > `timesfm_inference_0411.py` §3 동일
...
# MAGIC
# MAGIC > `timesfm_inference_0411.py` §3-1 동일: Pearson + Spearman 이중 히트맵
...
SCENARIOS = {
    "금리 인하 (-50bp)": {"us_treasury_10y": -0.50, "us_treasury_2y": -0.50},
    "금리 인상 (+50bp)": {"us_treasury_10y": +0.50, "us_treasury_2y": +0.50},
